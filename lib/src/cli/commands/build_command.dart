import 'dart:io';
import 'package:path/path.dart' as path;
import 'package:yaml/yaml.dart';
import '../../module_repository.dart';
import '../command.dart';

/// Command to generate modules.dart file with auto-registration code
class BuildCommand implements Command {
  @override
  Future<int> run(List<String> arguments) async {
    try {
      final projectRoot = Directory.current.path;

      // Check if we're in a Flutter project
      final pubspecPath = path.join(projectRoot, 'pubspec.yaml');
      final basePath = path.join(projectRoot, 'pubspec.yaml.base');

      // Detect modules path
      String? modulesPath;
      if (Directory(path.join(projectRoot, 'packages')).existsSync()) {
        modulesPath = 'packages';
      } else if (Directory(path.join(projectRoot, 'modules')).existsSync()) {
        modulesPath = 'modules';
      }

      // If base exists, auto-generate pubspec.yaml (modules never in git)
      if (File(basePath).existsSync()) {
        await _generateFromBase(
            basePath, pubspecPath, projectRoot, modulesPath);
      } else if (File(pubspecPath).existsSync()) {
        // Fallback: update existing pubspec.yaml
        if (modulesPath != null) {
          await _autoSyncPubspec(pubspecPath, projectRoot, modulesPath);
        }
      } else {
        print(
            'Error: Not in a Flutter project (pubspec.yaml or pubspec.yaml.base not found)');
        return 1;
      }

      // Generate route cache
      if (modulesPath != null) {
        await _generateRouteCache(projectRoot, modulesPath);
      }

      print('✓ Build complete');
      print('');
      print('Note: Module registration and route building are now handled');
      print('automatically by ModularApp. No manual setup needed!');
      print('');
      print('Your main.dart can now be as simple as:');
      print('  void main() {');
      print("    runApp(ModularApp(title: 'My App'));");
      print('  }');

      return 0;
    } catch (e) {
      print('Error generating modules.dart: $e');
      return 1;
    }
  }

  /// Generate pubspec.yaml from template (prevents git conflicts)
  Future<void> _autoSyncPubspec(
      String pubspecPath, String projectRoot, String modulesPath) async {
    try {
      // Use ModuleRepository to discover modules (same way as build command)
      final repository = ModuleRepository(localModulesPath: modulesPath);
      final modules = repository.scan();

      if (modules.isEmpty) {
        return;
      }

      // Get package names from discovered modules
      final discoveredModules = <String, String>{}; // packageName -> path
      for (final module in modules) {
        try {
          // Read module's pubspec.yaml to get actual package name
          final modulePubspecPath =
              path.join(projectRoot, modulesPath, module.alias, 'pubspec.yaml');
          if (File(modulePubspecPath).existsSync()) {
            final content = await File(modulePubspecPath).readAsString();
            final yaml = loadYaml(content) as Map;
            final packageName = yaml['name']?.toString();
            if (packageName != null) {
              final relativePath = path.join(modulesPath, module.alias);
              discoveredModules[packageName] = relativePath;
            }
          }
        } catch (e) {
          // Skip invalid modules
        }
      }

      if (discoveredModules.isEmpty) {
        return;
      }

      // Update existing pubspec.yaml
      final pubspecContent = await File(pubspecPath).readAsString();
      final pubspec = loadYaml(pubspecContent) as Map;
      final dependencies = pubspec['dependencies'] as Map? ?? {};

      // Check if we need to update
      bool needsUpdate = false;
      for (final entry in discoveredModules.entries) {
        final packageName = entry.key;
        final modulePath = entry.value;
        final currentDep = dependencies[packageName];

        if (currentDep == null ||
            (currentDep is Map && currentDep['path'] != modulePath)) {
          needsUpdate = true;
          break;
        }
      }

      if (!needsUpdate) {
        return; // Already synced
      }

      // Update pubspec.yaml by replacing modules section
      final moduleNames = discoveredModules.keys.toList()..sort();
      final lines = pubspecContent.split('\n');
      final result = <String>[];
      bool inDependencies = false;
      int depsIndent = 0;

      for (int i = 0; i < lines.length; i++) {
        final line = lines[i];
        final trimmed = line.trim();

        // Find dependencies section
        if (trimmed == 'dependencies:' || trimmed.startsWith('dependencies:')) {
          inDependencies = true;
          depsIndent = line.indexOf('dependencies');
          result.add(line);
          continue;
        }

        // In dependencies section
        if (inDependencies) {
          final currentIndent =
              line.isEmpty ? 999 : line.indexOf(RegExp(r'[^\s]'));

          // Detect start of modules section
          if (trimmed.contains('Feature modules') ||
              trimmed.contains('auto-generated')) {
            // Replace with new modules section
            result.add(
                '${' ' * (depsIndent + 2)}# Feature modules (auto-generated by modular_flutter)');
            for (final moduleName in moduleNames) {
              result.add('${' ' * (depsIndent + 2)}$moduleName:');
              result.add(
                  '${' ' * (depsIndent + 4)}path: ${discoveredModules[moduleName]}');
            }
            // Skip old module lines
            i++;
            while (i < lines.length) {
              final nextLine = lines[i];
              final nextIndent =
                  nextLine.isEmpty ? 999 : nextLine.indexOf(RegExp(r'[^\s]'));
              // Stop at next dependency or end of dependencies
              if (nextIndent <= depsIndent + 2 &&
                  !nextLine.trim().isEmpty &&
                  !nextLine.trim().startsWith('#')) {
                i--; // Back up
                break;
              }
              // Skip if it's a module entry
              if (moduleNames
                  .any((name) => nextLine.trim().startsWith('$name:'))) {
                i++;
                continue;
              }
              if (nextIndent > depsIndent + 2) {
                i++; // Skip indented lines (path: etc)
                continue;
              }
              break;
            }
            continue;
          }

          // Skip old module entries
          if (moduleNames.any((name) => trimmed.startsWith('$name:'))) {
            // Skip this module and its path line
            i++;
            while (i < lines.length) {
              final nextLine = lines[i];
              final nextIndent =
                  nextLine.isEmpty ? 999 : nextLine.indexOf(RegExp(r'[^\s]'));
              if (nextIndent <= depsIndent + 2) {
                i--; // Back up
                break;
              }
              i++;
            }
            continue;
          }

          // End of dependencies section
          if (currentIndent <= depsIndent &&
              trimmed.isNotEmpty &&
              !trimmed.startsWith('#')) {
            // Add modules section before this line
            result.add(
                '${' ' * (depsIndent + 2)}# Feature modules (auto-generated by modular_flutter)');
            for (final moduleName in moduleNames) {
              result.add('${' ' * (depsIndent + 2)}$moduleName:');
              result.add(
                  '${' ' * (depsIndent + 4)}path: ${discoveredModules[moduleName]}');
            }
            inDependencies = false;
          }
        }

        result.add(line);
      }

      // Write updated pubspec.yaml
      final updatedContent = result.join('\n');
      if (updatedContent != pubspecContent) {
        await File(pubspecPath).writeAsString(updatedContent);
        print(
            '✓ Auto-synced pubspec.yaml with ${discoveredModules.length} discovered modules');
      }
    } catch (e) {
      // Silently fail - don't break the build process
      print('Warning: Could not auto-sync pubspec.yaml: $e');
    }
  }

  /// Generate pubspec.yaml from base (modules never in git)
  Future<void> _generateFromBase(String basePath, String pubspecPath,
      String projectRoot, String? modulesPath) async {
    try {
      // Read base file
      final baseContent = await File(basePath).readAsString();

      if (modulesPath == null) {
        // No modules, just copy base
        await File(pubspecPath).writeAsString(baseContent);
        return;
      }

      // Discover modules
      final repository = ModuleRepository(localModulesPath: modulesPath);
      final modules = repository.scan();

      if (modules.isEmpty) {
        // No modules, just copy base
        await File(pubspecPath).writeAsString(baseContent);
        return;
      }

      // Get package names
      final discoveredModules = <String, String>{};
      for (final module in modules) {
        try {
          final modulePubspecPath =
              path.join(projectRoot, modulesPath, module.alias, 'pubspec.yaml');
          if (File(modulePubspecPath).existsSync()) {
            final content = await File(modulePubspecPath).readAsString();
            final yaml = loadYaml(content) as Map;
            final packageName = yaml['name']?.toString();
            if (packageName != null) {
              final relativePath = path.join(modulesPath, module.alias);
              discoveredModules[packageName] = relativePath;
            }
          }
        } catch (e) {
          // Skip invalid modules
        }
      }

      // Insert modules after modular_flutter
      final baseLines = baseContent.split('\n');
      final result = <String>[];
      bool inserted = false;

      for (final line in baseLines) {
        result.add(line);

        // Insert modules after modular_flutter
        if (!inserted && line.trim().startsWith('modular_flutter:')) {
          result.add('');
          result.add('  # Feature modules (auto-generated by modular_flutter)');
          final moduleNames = discoveredModules.keys.toList()..sort();
          for (final moduleName in moduleNames) {
            result.add('  $moduleName:');
            result.add('    path: ${discoveredModules[moduleName]}');
          }
          inserted = true;
        }
      }

      // Write generated pubspec.yaml
      await File(pubspecPath).writeAsString(result.join('\n'));
    } catch (e) {
      print('Warning: Could not generate pubspec.yaml: $e');
    }
  }

  /// Generate route cache file that imports all route widgets
  Future<void> _generateRouteCache(
      String projectRoot, String modulesPath) async {
    try {
      final repository = ModuleRepository(localModulesPath: modulesPath);
      final modules = repository.scan();

      if (modules.isEmpty) {
        return;
      }

      // Collect all routes from all modules
      final routeImports = <String, Map<String, String>>{};
      // Map: widgetPath -> {package, import, className}

      for (final module in modules) {
        if (!module.enabled) continue;

        // Read module's pubspec.yaml to get package name
        final modulePubspecPath =
            path.join(projectRoot, modulesPath, module.alias, 'pubspec.yaml');
        if (!File(modulePubspecPath).existsSync()) continue;

        final pubspecContent = await File(modulePubspecPath).readAsString();
        final pubspec = loadYaml(pubspecContent) as Map;
        final packageName = pubspec['name']?.toString();
        if (packageName == null) continue;

        // Process routes from module.yaml
        for (final routeMap in module.routes) {
          final widgetPath = routeMap['widget']?.toString();
          if (widgetPath == null) continue;

          // Parse widget path like "auth_module.routes.AuthRoute"
          final parts = widgetPath.split('.');
          if (parts.length < 3) continue;

          final namespace = parts[1]; // e.g., 'routes'
          final className = parts[2]; // e.g., 'AuthRoute'
          final fileName = _toSnakeCase(className);

          routeImports[widgetPath] = {
            'package': packageName,
            'import': 'package:$packageName/$namespace/$fileName.dart',
            'className': className,
          };
        }
      }

      if (routeImports.isEmpty) {
        return;
      }

      // Generate route cache file in lib directory so it can be imported
      final libDir = Directory(path.join(projectRoot, 'lib'));
      if (!libDir.existsSync()) {
        libDir.createSync(recursive: true);
      }

      final cacheFile = File(path.join(libDir.path, '.modular_flutter', 'route_cache.g.dart'));
      final cacheDir = cacheFile.parent;
      if (!cacheDir.existsSync()) {
        cacheDir.createSync(recursive: true);
      }
      final buffer = StringBuffer();

      buffer.writeln('// GENERATED FILE - DO NOT EDIT MANUALLY');
      buffer.writeln('// This file is auto-generated by `dart run modular_flutter build`');
      buffer.writeln('// It contains route mappings for all discovered modules');
      buffer.writeln('');
      buffer.writeln("import 'package:flutter/material.dart';");
      buffer.writeln('');

      // Generate imports
      final uniqueImports = <String, String>{};
      for (final entry in routeImports.entries) {
        final import = entry.value['import']!;
        final className = entry.value['className']!;
        uniqueImports[import] = className;
      }

      for (final import in uniqueImports.keys.toList()..sort()) {
        buffer.writeln("import '$import';");
      }

      buffer.writeln('');
      buffer.writeln("import 'package:modular_flutter/src/build/route_cache_stub.dart';");
      buffer.writeln('');
      buffer.writeln('/// Auto-generated route cache initialization');
      buffer.writeln('/// This file populates RouteCache.cache with all discovered routes');
      buffer.writeln('/// Import this file in your main.dart to enable route resolution');
      buffer.writeln('void initModularRouteCache() {');
      buffer.writeln('  RouteCache.cache.addAll({');

      // Generate route mappings
      for (final entry in routeImports.entries) {
        final widgetPath = entry.key;
        final className = entry.value['className']!;
        buffer.writeln("    '$widgetPath': (context) => const $className(),");
      }

      buffer.writeln('  });');
      buffer.writeln('}');

      await cacheFile.writeAsString(buffer.toString());
      print('✓ Generated route cache with ${routeImports.length} routes');
      
      // Auto-update main.dart to import and initialize route cache
      await _updateMainDart(projectRoot, cacheFile);
    } catch (e) {
      print('Warning: Could not generate route cache: $e');
    }
  }

  /// Update main.dart to import and initialize route cache
  Future<void> _updateMainDart(String projectRoot, File cacheFile) async {
    try {
      final mainDartPath = path.join(projectRoot, 'lib', 'main.dart');
      final mainDartFile = File(mainDartPath);
      
      if (!mainDartFile.existsSync()) {
        return; // No main.dart to update
      }

      final content = await mainDartFile.readAsString();
      
      // Calculate relative import path
      final relativePath = path.relative(cacheFile.path, from: path.join(projectRoot, 'lib'));
      final importPath = relativePath.replaceAll('\\', '/').replaceFirst('.dart', '');
      
      // Check if already imported
      if (content.contains("import '.modular_flutter/route_cache.g.dart';") ||
          content.contains("import '$importPath';")) {
        // Check if initModularRouteCache is called
        if (content.contains('initModularRouteCache()')) {
          return; // Already set up
        }
      }
      
      // Add import after other imports
      final lines = content.split('\n');
      final result = <String>[];
      bool importsDone = false;
      bool initAdded = false;
      
      for (int i = 0; i < lines.length; i++) {
        final line = lines[i];
        result.add(line);
        
        // After imports, add route cache import
        if (!importsDone && line.trim().isEmpty && i > 0 && lines[i-1].trim().isNotEmpty) {
          final prevLine = lines[i-1].trim();
          if (prevLine.startsWith('import ') || prevLine.startsWith('//')) {
            // Still in imports section
            continue;
          } else if (!prevLine.startsWith('import ')) {
            // Imports are done, add route cache import
            result.add("import '.modular_flutter/route_cache.g.dart';");
            importsDone = true;
          }
        }
        
        // Add initModularRouteCache call before runApp
        if (!initAdded && line.contains('runApp(')) {
          // Find the line before runApp
          result.insert(result.length - 1, '  initModularRouteCache(); // Initialize route cache');
          initAdded = true;
        }
      }
      
      // If we didn't find a good place, add at the end of imports
      if (!importsDone) {
        for (int i = 0; i < result.length; i++) {
          if (result[i].trim().startsWith('void main()')) {
            result.insert(i, "import '.modular_flutter/route_cache.g.dart';");
            result.insert(i, '');
            break;
          }
        }
      }
      
      // If init not added, add it at start of main
      if (!initAdded) {
        for (int i = 0; i < result.length; i++) {
          if (result[i].trim().startsWith('void main()')) {
            // Find the first line inside main
            for (int j = i + 1; j < result.length; j++) {
              if (result[j].trim().startsWith('WidgetsFlutterBinding.ensureInitialized()')) {
                result.insert(j + 1, '  initModularRouteCache(); // Initialize route cache');
                break;
              }
            }
            break;
          }
        }
      }
      
      final updatedContent = result.join('\n');
      if (updatedContent != content) {
        await mainDartFile.writeAsString(updatedContent);
        print('✓ Updated main.dart to import route cache');
      }
    } catch (e) {
      print('Warning: Could not update main.dart: $e');
    }
  }

  /// Convert StudlyCase to snake_case
  static String _toSnakeCase(String input) {
    return input
        .replaceAllMapped(
          RegExp(r'([A-Z])'),
          (match) => '_${match.group(1)!.toLowerCase()}',
        )
        .replaceFirst(RegExp(r'^_'), '');
  }
}
